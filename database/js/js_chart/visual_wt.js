if( typeof wt_menu == 'undefined' )wt_menu = function(t){return document.getElementById(t)};

if( typeof wt_menuC == 'undefined' )wt_menuC = function(t){return document.createElement(t)};
...
ScrTabCtrl.prototype = {
    ...
    Init: function (aData,callbacks)
    {
        ...
        var tempDls = wt_menu("wmt_" + aData.id).getElementsByTagName("DL");
...
TabSwitch.prototype = {
    ...
    {
        this.tabs = [];
        var tempTabs1 = tabs.childNodes;
        for(var i = 0;i < tempTabs1.length;i++)
        {
            if(tempTabs1[i].nodeType == 1)
            {
                this.tabs.push(tempTabs1[i]);
            }
        };
...
WmtHq_Ctrl.prototype = {
    ...
    Init: function (oWmtdata)
    {
        ...
        this._tabCtrl = new TabSwitch($("wmt_tabs"),$("wmt_contents"),null,null,[this.show.Bind(this,this.oWmtdata.china),this.show.Bind(this,this.oWmtdata.asia),this.show.Bind(this,this.oWmtdata.america),this.show.Bind(this,this.oWmtdata.euro)]);
    ...
    _start: function ()
    {
        ...
        {
            this._tabCtrl.showTab(2,this.show.Bind(this,this.oWmtdata.america));
    ...
    show: function (aData)
    {
        ...
        if(!this.oWmtdata[this._sMarket].hqApi)
        {
            ...
            this.oWmtdata[this._sMarket].hqApi = this._hqUrl + _hqcodeNow.join(",");
        }

        if(!this.oWmtdata[this._sMarket]._containersNow)
        {
            this.oWmtdata[this._sMarket]._containersNow = [];
            var _tmpchilds = wt_menu("wmt_" + aData.id).childNodes;
    ...
    _loadhq: function ()
    {
        ...
        this._loader.load(this.oWmtdata[this._sMarket].hqApi.replace("@RN@",(new Date()).getTime()),this._fill.Bind(this));
    },
    ...
    _parseData: function (sData,aSet,index)
    {
        ...
        switch(aSet[3])
        {
            ...
            case "npqh":
                ...
                var _redorgreen = this._setColorClass(_change,aSet[4]);
                var _price = (sData[8] * 1).toFixed(2);
...
WmtHq_Ctrl_Multi.prototype.Init = function (oWmtdata,objData)
{
    ...
    if(this._tabCtrl){
    	this._tabCtrl.Init(wt_menu(this.objData.wmt_tabs),wt_menu(this.objData.wmt_contents),null,null,_callArr);
    }else{
    	 this._tabCtrl = new TabSwitch(wt_menu(this.objData.wmt_tabs),wt_menu(this.objData.wmt_contents),null,null,_callArr);
    }
...
SINAHQ_LOADER.prototype = {
	...
	load: function(url, callback){
		...
		if(wt_menu("wmt_loader"))wt_menu("wmt_loader").appendChild(this._oScript);
	}
} 
