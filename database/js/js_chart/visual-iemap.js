...
function visual_hqgszs(param_){
	...
	var cfg={
		...
		LEDENGCOLOR:['rgb(35,166,76)','rgb(76,187,115)','rgb(124,210,154)','rgb(211,237,212)','#F3F3F3','rgb(255,223,214)','rgb(255,128,128)','rgb(255,86,86)','rgb(245,69,69)'],
		fdownColor:'#23a64c',
		fupColor:'#f54545',
	...
	function sMap(){
		...
		var _basicMap=cfg.dom.basicMap,_clickLayer=createElement('div'),_wmt_mapworld=getDocument('wmt_mapworld'),
		    ...
		    _popup_wm5=getDocument('popup_wm5');_popup_jjsj_a=getDocument('popup_jjsj_a');
		...
		function _showPopup(e_,cindex_){
    		...
    		if(_curData.id==="USA"){//redup greenup
    			...
    			oWmtdata.mapworld.hqApi="http://hq.sinajs.cn/rn=@RN@&list=b_UKX"
    			oWmtdata.mapworld.set=[
    				["b_UKX","英国富时100指数","http://image.sinajs.cn/newchart/futures/forex/min5_m_hollow/FT100.gif","bb","redup",""]
    			]
    		}else if(_curData.id==="JPN"){
    			_popup_wm0.style.display=_popup_wm3.style.display='block';
    			
    			oWmtdata.mapworld.hqApi="http://hq.sinajs.cn/rn=@RN@&list=b_NKY"
    			oWmtdata.mapworld.set=[
    				["b_NKY","日经指数","http://image.sinajs.cn/newchart/futures/forex/min5_m_hollow/NIXI.gif","bb","redup",""],
    			]
    		}else{
    			if(cindex_!==1000){
    				_popup_wm0.style.display=_popup_wm3.style.display='block';
		...
		function _createDom(){
		    ...
		    var legendcontainer=fcontainer.append('div');
		   		legendcontainer.style('styleFloat','right')
			...
			for(var i=0;i<legendNum;i++){
				var legenddiv=legendcontainer.append('div')
							...
							.style("backgroundColor",cfg.LEDENGCOLOR[i])
			...
			var updownbtn=document.createElement('div');
				...
				_clickLayer.style.top=-_h-128+'px';
				_clickLayer.style.width=_w+'px';
				_clickLayer.style.height=_h+'px';
				_clickLayer.style.background="url('mom_transparentBg.png')";
