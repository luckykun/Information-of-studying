...
(function(){
    var bond_zqhq_gsz={
        ...
        initEvents:function(){
            ...
            for(var i=0; i<thList.length;i++){
                ...
                if($(thList[i]).find("span").length!=0){
                    //为含有span标签的th标签绑定事件
                    $(thList[i]).find("span").attr("onclick","bond_zqhq_gsz.fieldSort(this)");
                }
            }
        },
        ...
        fieldSort:function(element){
            ...
            bond_zqhq_gsz.totalArray=bond_zqhq_gsz.arrSort(bond_zqhq_gsz.totalArray,index,bond_zqhq_gsz.asc);
            bond_zqhq_gsz.insertTableData();
        },
        ...
        getTableData:function(){
            var theURL="http://finance.sina.com.cn/data_center/zq/bond_zqhq_gsz.js";

            bond_zqhq_gsz.tableDataScript=document.createElement("script");
            bond_zqhq_gsz.tableDataScript.setAttribute("src",theURL);
            document.body.insertBefore(bond_zqhq_gsz.tableDataScript, document.body.childNodes[0]);

            //如果是IE
            if(document.all){
                bond_zqhq_gsz.tableDataScript.onreadystatechange=function(){
                    if(bond_zqhq_gsz.tableDataScript.readyState=="loaded" || bond_zqhq_gsz.tableDataScript.readyState=="complete") {
                        bond_zqhq_gsz.theTableData(bond_zqhq_gsz_data);
                    }
                }
            }
            else{
                bond_zqhq_gsz.tableDataScript.onload=function(){
                    bond_zqhq_gsz.theTableData(bond_zqhq_gsz_data);
                }
            }
        },
        ...
        theTableData:function(data){
            try{
                ...
                document.body.removeChild(bond_zqhq_gsz.tableDataScript);
            }
            catch(error){
                //do nothing
            }
            ...
            bond_zqhq_gsz.pushIntoArray(data[0].items);
        },
        ...
        pushIntoArray:function(dataArr){
            ...
            for(var i=0;i<dataArr.length;i++){
                ...
                currentArray.push(parseFloat(dataArr[i][17]).toFixed(2)+'%');//到期收益率
                ...
                bond_zqhq_gsz.totalArray.push(currentArray);
            }
        ...
        insertTableData:function(){
            var totalLength=bond_zqhq_gsz.totalArray.length;
            var tableHTML="";

            for(var i=0;i<totalLength;i++){
                var trDataArr=bond_zqhq_gsz.totalArray[i];
    ...
    window.bond_zqhq_gsz=bond_zqhq_gsz;
})();