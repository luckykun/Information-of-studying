...
(function(){
    var futures_qhhq_jytd={
        ...
        initEvents:function(){
            //为pageDiv里的分页标签们添加事件
            $("#pageDiv_0 input[type=button]").attr("onclick","futures_qhhq_jytd.gotoPage()");
            $("#pageDiv_0 .firstPageSpan").attr("onclick","futures_qhhq_jytd.firstPage()");
            $("#pageDiv_0 .lastPageSpan").attr("onclick","futures_qhhq_jytd.lastPage()");
            $("#pageDiv_0 .previousPageSpan").attr("onclick","futures_qhhq_jytd.previousPage()");
            $("#pageDiv_0 .nextPageSpan").attr("onclick","futures_qhhq_jytd.nextPage()");
            ...
            for(var i=0; i<thList.length;i++){
                ...
                if($(thList[i]).find("span").length!=0){
                    //为含有span标签的th标签绑定事件
                    $(thList[i]).find("span").attr("onclick","futures_qhhq_jytd.fieldSort(this)");
                }
            }

            //为每页显示条数绑定事件
            $("#numberDiv_0").find("a").attr("onclick","futures_qhhq_jytd.changeNumber(this)");
        },
        ...
        gotoPage:function(){
            ...
            if(parseInt(inputPage)){
                ...
                futures_qhhq_jytd.insertTableData();
            }
            else{
                //alert("输入的数据有误。");
                showmsg("输入的数据有误。",1);
        ...
        changeNumber:function(element){
            ...
            var totalPage=Math.ceil(futures_qhhq_jytd.dataCount/number);//计算总页数
        ...
        fieldSort:function(element){
            ...
            futures_qhhq_jytd.totalArray=futures_qhhq_jytd.arrSort(futures_qhhq_jytd.totalArray,index,futures_qhhq_jytd.asc);
            futures_qhhq_jytd.insertTableData();
        },
        ...
        getTableData:function(){
            var theURL="http://vip.stock.finance.sina.com.cn/d/api/openapi_proxy.php/?__s=[[%22jytd%22]]&callback=futures_qhhq_jytd.theTableData";
            ...
            /*FDC_DC.theRequest=$.ajax({
                ...
                success:function(data){
                    futures_qhhq_jytd.getTotalPage(data[0].count);

                    //将需要在表格中显示的字段，按顺序放入数组中
                    futures_qhhq_jytd.pushIntoArray(data[0].items);
                    futures_qhhq_jytd.insertTableData();
                }
            });*/

            futures_qhhq_jytd.tableDataScript=document.createElement("script");
            futures_qhhq_jytd.tableDataScript.setAttribute("src",theURL);
            document.body.insertBefore(futures_qhhq_jytd.tableDataScript, document.body.childNodes[0]);
        },

        theTableData:function(data){
            try{
                //删除掉刚才请求接口时创建的script的标签
                document.body.removeChild(futures_qhhq_jytd.tableDataScript);
            }
            catch(error){
                //do nothing
            }
        ...
        pushIntoArray:function(dataArr){
            ...
            for(var i=0;i<dataArr.length;i++){
                ...
                currentArray.push(dataArr[i][2]);//品种
                currentArray.push(dataArr[i][1]);//名称
                currentArray.push(dataArr[i][3]);//最新价
                currentArray.push(dataArr[i][6]);//开盘价
                currentArray.push(dataArr[i][7]);//最高价
                currentArray.push(dataArr[i][8]);//最低价
                currentArray.push(dataArr[i][18]);//涨跌幅
                currentArray.push(dataArr[i][9]);//昨收价
                currentArray.push(dataArr[i][14]);//总成交量
                currentArray.push(dataArr[i][15]);//总成交额
                currentArray.push(dataArr[i][16]+"<br/>"+dataArr[i][17]);//更新时间
                currentArray.push("查看");//历史交易

                currentArray.push(dataArr[i][0]);//代码

                futures_qhhq_jytd.totalArray.push(currentArray);
            }
        },
        ...
        insertTableData:function(){
            ...
            if(page*number>futures_qhhq_jytd.totalArray.length){
                totalLength=futures_qhhq_jytd.totalArray.length;
            }
            else{
            ...
            for(var i=beginIndex;i<totalLength;i++){
                var trDataArr=futures_qhhq_jytd.totalArray[i];

                tableHTML+="<tr>";
                for(var j=0;j<trDataArr.length-1;j++){
                    switch(j){
                        case 11:
                            tableHTML+="<td><a href='http://vip.stock.finance.sina.com.cn/q/view/vGold_History.php?breed="+trDataArr[trDataArr.length-1]+"' target='_blank'>"+trDataArr[11]+"</a></td>";//代码
                            break;
                        default :
        ...
        getTotalPage:function(count){
            futures_qhhq_jytd.dataCount=parseInt(count);
    ...
    window.futures_qhhq_jytd=futures_qhhq_jytd;
})();
