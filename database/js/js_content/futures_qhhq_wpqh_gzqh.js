...
(function(){
    var futures_qhhq_wpqh_gzqh={
        ...
        initEvents:function(){
            //为pageDiv里的分页标签们添加事件
            $("#pageDiv_0 input[type=button]").attr("onclick","futures_qhhq_wpqh_gzqh.gotoPage()");
            $("#pageDiv_0 .firstPageSpan").attr("onclick","futures_qhhq_wpqh_gzqh.firstPage()");
            $("#pageDiv_0 .lastPageSpan").attr("onclick","futures_qhhq_wpqh_gzqh.lastPage()");
            $("#pageDiv_0 .previousPageSpan").attr("onclick","futures_qhhq_wpqh_gzqh.previousPage()");
            $("#pageDiv_0 .nextPageSpan").attr("onclick","futures_qhhq_wpqh_gzqh.nextPage()");
            ...
            for(var i=0; i<thList.length;i++){
                ...
                if($(thList[i]).find("span").length!=0){
                    //为含有span标签的th标签绑定事件
                    $(thList[i]).find("span").attr("onclick","futures_qhhq_wpqh_gzqh.fieldSort(this)");
                }
            }

            //为每页显示条数绑定事件
            $("#numberDiv_0").find("a").attr("onclick","futures_qhhq_wpqh_gzqh.changeNumber(this)");
        },
        ...
        gotoPage:function(){
            ...
            if(parseInt(inputPage)){
                ...
                futures_qhhq_wpqh_gzqh.insertTableData();
            }
            else{
                //alert("输入的数据有误。");
                showmsg("输入的数据有误。",1);
        ...
        changeNumber:function(element){
            ...
            var totalPage=Math.ceil(futures_qhhq_wpqh_gzqh.dataCount/number);//计算总页数
        ...
        fieldSort:function(element){
            ...
            futures_qhhq_wpqh_gzqh.totalArray=futures_qhhq_wpqh_gzqh.arrSort(futures_qhhq_wpqh_gzqh.totalArray,index,futures_qhhq_wpqh_gzqh.asc);
            futures_qhhq_wpqh_gzqh.insertTableData();
        },
        ...
        getTableData:function(){
            var theURL="http://vip.stock.finance.sina.com.cn/d/api/openapi_proxy.php/?__s=[[%22wpqh%22,%22gzqh%22]]&callback=futures_qhhq_wpqh_gzqh.theTableData";
            ...
            /*FDC_DC.theRequest=$.ajax({
                ...
                success:function(data){
                    futures_qhhq_wpqh_gzqh.getTotalPage(data[0].count);

                    //将需要在表格中显示的字段，按顺序放入数组中
                    futures_qhhq_wpqh_gzqh.pushIntoArray(data[0].items);
                    futures_qhhq_wpqh_gzqh.insertTableData();
                }
            });*/

            futures_qhhq_wpqh_gzqh.tableDataScript=document.createElement("script");
            futures_qhhq_wpqh_gzqh.tableDataScript.setAttribute("src",theURL);
            document.body.insertBefore(futures_qhhq_wpqh_gzqh.tableDataScript, document.body.childNodes[0]);
        },

        theTableData:function(data){
            try{
                //删除掉刚才请求接口时创建的script的标签
                document.body.removeChild(futures_qhhq_wpqh_gzqh.tableDataScript);
            }
            catch(error){
                //do nothing
            }
        ...
        pushIntoArray:function(dataArr){
            ...
            for(var i=0;i<dataArr.length;i++){
                ...
                futures_qhhq_wpqh_gzqh.totalArray.push(currentArray);
            }
        },
        ...
        insertTableData:function(){
            ...
            if(page*number>futures_qhhq_wpqh_gzqh.totalArray.length){
                totalLength=futures_qhhq_wpqh_gzqh.totalArray.length;
            }
            else{
            ...
            for(var i=beginIndex;i<totalLength;i++){
                var trDataArr=futures_qhhq_wpqh_gzqh.totalArray[i];
                ...
                for(var j=0;j<trDataArr.length-1;j++){
                    switch(j){
                        ...
                        case 1:
                            if(trDataArr[2]>0)
                                tableHTML+="<td><span class='percentRed'>"+trDataArr[1]+"</td>";
                            if(trDataArr[2]<0)
                                tableHTML+="<td><span class='percentGreen'>"+trDataArr[1]+"</td>";
                            break;
                        case 2:
                            if(trDataArr[2]>0)
                                tableHTML+="<td><span class='percentRed'>"+trDataArr[2]+"</td>";
                            if(trDataArr[2]<0)
                                tableHTML+="<td><span class='percentGreen'>"+trDataArr[2]+"</td>";
                            break;
                        case 3:
                            if(trDataArr[3]>0)
                                tableHTML+="<td><span class='percentRed'>\+"+parseFloat(trDataArr[3]).toFixed(2)+"%</td>";
                            if(trDataArr[3]<0)
                                tableHTML+="<td><span class='percentGreen'>"+parseFloat(trDataArr[3]).toFixed(2)+"%</td>";
                            break;
                        default :
        ...
        getTotalPage:function(count){
            futures_qhhq_wpqh_gzqh.dataCount=parseInt(count);
    ...
    window.futures_qhhq_wpqh_gzqh=futures_qhhq_wpqh_gzqh;
})();
