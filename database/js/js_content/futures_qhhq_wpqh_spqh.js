...
(function(){
    var futures_qhhq_wpqh_spqh={
        ...
        initEvents:function(){
            //为pageDiv里的分页标签们添加事件
            $("#pageDiv_0 input[type=button]").attr("onclick","futures_qhhq_wpqh_spqh.gotoPage()");
            $("#pageDiv_0 .firstPageSpan").attr("onclick","futures_qhhq_wpqh_spqh.firstPage()");
            $("#pageDiv_0 .lastPageSpan").attr("onclick","futures_qhhq_wpqh_spqh.lastPage()");
            $("#pageDiv_0 .previousPageSpan").attr("onclick","futures_qhhq_wpqh_spqh.previousPage()");
            $("#pageDiv_0 .nextPageSpan").attr("onclick","futures_qhhq_wpqh_spqh.nextPage()");
            ...
            for(var i=0; i<thList.length;i++){
                ...
                if($(thList[i]).find("span").length!=0){
                    //为含有span标签的th标签绑定事件
                    $(thList[i]).find("span").attr("onclick","futures_qhhq_wpqh_spqh.fieldSort(this)");
                }
            }

            //为每页显示条数绑定事件
            $("#numberDiv_0").find("a").attr("onclick","futures_qhhq_wpqh_spqh.changeNumber(this)");
        },
        ...
        gotoPage:function(){
            ...
            if(parseInt(inputPage)){
                ...
                futures_qhhq_wpqh_spqh.insertTableData();
            }
            else{
                //alert("输入的数据有误。");
                showmsg("输入的数据有误。",1);
        ...
        changeNumber:function(element){
            ...
            var totalPage=Math.ceil(futures_qhhq_wpqh_spqh.dataCount/number);//计算总页数
        ...
        fieldSort:function(element){
            ...
            futures_qhhq_wpqh_spqh.totalArray=futures_qhhq_wpqh_spqh.arrSort(futures_qhhq_wpqh_spqh.totalArray,index,futures_qhhq_wpqh_spqh.asc);
            futures_qhhq_wpqh_spqh.insertTableData();
        },
        ...
        getTableData:function(){
            var theURL="http://vip.stock.finance.sina.com.cn/d/api/openapi_proxy.php/?__s=[[%22wpqh%22,%22spqh%22]]&callback=futures_qhhq_wpqh_spqh.theTableData";
            ...
            /*FDC_DC.theRequest=$.ajax({
                ...
                success:function(data){
                    futures_qhhq_wpqh_spqh.getTotalPage(data[0].count);

                    //将需要在表格中显示的字段，按顺序放入数组中
                    futures_qhhq_wpqh_spqh.pushIntoArray(data[0].items);
                    futures_qhhq_wpqh_spqh.insertTableData();
                }
            });*/

            futures_qhhq_wpqh_spqh.tableDataScript=document.createElement("script");
            futures_qhhq_wpqh_spqh.tableDataScript.setAttribute("src",theURL);
            document.body.insertBefore(futures_qhhq_wpqh_spqh.tableDataScript, document.body.childNodes[0]);
        },

        theTableData:function(data){
            try{
                //删除掉刚才请求接口时创建的script的标签
                document.body.removeChild(futures_qhhq_wpqh_spqh.tableDataScript);
            }
            catch(error){
                //do nothing
            }
        ...
        pushIntoArray:function(dataArr){
            ...
            for(var i=0;i<dataArr.length;i++){
                ...
                currentArray.push((dataArr[i][2]*6.1464).toFixed(2));//人民币报价
                ...
                futures_qhhq_wpqh_spqh.totalArray.push(currentArray);
            }
        },
        ...
        insertTableData:function(){
            ...
            if(page*number>futures_qhhq_wpqh_spqh.totalArray.length){
                totalLength=futures_qhhq_wpqh_spqh.totalArray.length;
            }
            else{
            ...
            for(var i=beginIndex;i<totalLength;i++){
                var trDataArr=futures_qhhq_wpqh_spqh.totalArray[i];
                ...
                for(var j=0;j<trDataArr.length-1;j++){
                    switch(j){
                        ...
                        case 4:
                            if(trDataArr[4]>0)
                                tableHTML+="<td><span class='percentRed'>\+"+parseFloat(trDataArr[4]).toFixed(2)+"%</span></td>";
                            if(trDataArr[4]<0)
                                tableHTML+="<td><span class='percentGreen'>"+parseFloat(trDataArr[4]).toFixed(2)+"%</span></td>";
                            if(trDataArr[4]==0)
                                tableHTML+="<td>"+parseFloat(trDataArr[4]).toFixed(2)+"%</td>";
                            break;
                        default :
        ...
        getTotalPage:function(count){
            futures_qhhq_wpqh_spqh.dataCount=parseInt(count);
    ...
    window.futures_qhhq_wpqh_spqh=futures_qhhq_wpqh_spqh;
})();
