...
(function(){
    var forex_whhq_whqh={
        ...
        initEvents:function(){
            //为pageDiv里的分页标签们添加事件
            $("#pageDiv_0 input[type=button]").attr("onclick","forex_whhq_whqh.gotoPage()");
            $("#pageDiv_0 .firstPageSpan").attr("onclick","forex_whhq_whqh.firstPage()");
            $("#pageDiv_0 .lastPageSpan").attr("onclick","forex_whhq_whqh.lastPage()");
            $("#pageDiv_0 .previousPageSpan").attr("onclick","forex_whhq_whqh.previousPage()");
            $("#pageDiv_0 .nextPageSpan").attr("onclick","forex_whhq_whqh.nextPage()");
            ...
            for(var i=0; i<thList.length;i++){
                ...
                if($(thList[i]).find("span").length!=0){
                    //为含有span标签的th标签绑定事件
                    $(thList[i]).find("span").attr("onclick","forex_whhq_whqh.fieldSort(this)");
                }
            }

            //为每页显示条数绑定事件
            $("#numberDiv_0").find("a").attr("onclick","forex_whhq_whqh.changeNumber(this)");
        },
        ...
        gotoPage:function(){
            ...
            if(parseInt(inputPage)){
                ...
                forex_whhq_whqh.insertTableData();
            }
            else{
                //alert("输入的数据有误。");
                showmsg("输入的数据有误。",1);
        ...
        changeNumber:function(element){
            ...
            var totalPage=Math.ceil(forex_whhq_whqh.dataCount/number);//计算总页数
        ...
        fieldSort:function(element){
            ...
            forex_whhq_whqh.totalArray=forex_whhq_whqh.arrSort(forex_whhq_whqh.totalArray,index,forex_whhq_whqh.asc);
            forex_whhq_whqh.insertTableData();
        },
        ...
        /*getTableData:function(){
            var theURL="http://vip.stock.finance.sina.com.cn/d/api/openapi_proxy.php/?__s=[[%22wpqh%22,%22whqh%22]]";
            ...
            FDC_DC.theRequest=$.ajax({
                ...
                success:function(data){
                    forex_whhq_whqh.getTotalPage(data[0].count);

                    //将需要在表格中显示的字段，按顺序放入数组中
                    forex_whhq_whqh.pushIntoArray(data[0].items);
                    forex_whhq_whqh.insertTableData();
                }
            });
        },*/
        ...
        getTableData:function(){
            var theURL="http://vip.stock.finance.sina.com.cn/d/api/openapi_proxy.php/?__s=[[%22wpqh%22,%22whqh%22]]&callback=forex_whhq_whqh.theTableData";

            forex_whhq_whqh.tableDataScript=document.createElement("script");
            forex_whhq_whqh.tableDataScript.setAttribute("src",theURL);
            document.body.insertBefore(forex_whhq_whqh.tableDataScript, document.body.childNodes[0]);
        },

        theTableData:function(data){
            try{
                //删除掉刚才请求接口时创建的script的标签
                document.body.removeChild(forex_whhq_whqh.tableDataScript);
            }
            catch(error){
                //do nothing
            }
        ...
        pushIntoArray:function(dataArr){
            ...
            for(var i=0;i<dataArr.length;i++){
                ...
                currentArray.push(dataArr[i][13]);//买量
                currentArray.push(dataArr[i][14]);//卖量
                currentArray.push(dataArr[i][12]);//持仓量

                forex_whhq_whqh.totalArray.push(currentArray);
            }
        },
        ...
        insertTableData:function(){
            ...
            if(page*number>forex_whhq_whqh.totalArray.length){
                totalLength=forex_whhq_whqh.totalArray.length;
            }
            else{
            ...
            for(var i=beginIndex;i<totalLength;i++){
                var trDataArr=forex_whhq_whqh.totalArray[i];

                tableHTML+="<tr>";
                for(var j=0;j<trDataArr.length;j++){
                    switch(j){
                        case 0:
                            tableHTML+="<td><a href='http://finance.sina.com.cn/money/future/quote_hf.html?"+trDataArr[0]+"' target='_blank'>"+trDataArr[0]+"</a></td>";
                            break;
                        case 1:
                            tableHTML+="<td><a href='http://finance.sina.com.cn/money/future/quote_hf.html?"+trDataArr[0]+"' target='_blank'>"+trDataArr[1]+"</a></td>";
                            break;
                        case 2:
                            if(trDataArr[2]>0)
        ...
        getTotalPage:function(count){
            forex_whhq_whqh.dataCount=parseInt(count);
    ...
    window.forex_whhq_whqh=forex_whhq_whqh;
})();
