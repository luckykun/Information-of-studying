...
(function(){
    var forex_whhq={
        ...
        initEvents:function(){
            //为pageDiv里的分页标签们添加事件
            $("#pageDiv_0 input[type=button]").attr("onclick","forex_whhq.gotoPage()");
            $("#pageDiv_0 .firstPageSpan").attr("onclick","forex_whhq.firstPage()");
            $("#pageDiv_0 .lastPageSpan").attr("onclick","forex_whhq.lastPage()");
            $("#pageDiv_0 .previousPageSpan").attr("onclick","forex_whhq.previousPage()");
            $("#pageDiv_0 .nextPageSpan").attr("onclick","forex_whhq.nextPage()");
            ...
            for(var i=0; i<thList.length;i++){
                ...
                if($(thList[i]).find("span").length!=0){
                    //为含有span标签的th标签绑定事件
                    $(thList[i]).find("span").attr("onclick","forex_whhq.fieldSort(this)");
                }
            }

            //为每页显示条数绑定事件
            $("#numberDiv_0").find("a").attr("onclick","forex_whhq.changeNumber(this)");
        },
        ...
        changeNumber:function(element){
            ...
            var totalPage=Math.ceil(forex_whhq.dataCount/number);//计算总页数
        ...
        fieldSort:function(element){
            ...
            forex_whhq.totalArray=forex_whhq.arrSort(forex_whhq.totalArray,index,forex_whhq.asc);
            forex_whhq.insertTableData();
        },

        ajaxTotalPage:function(){
            var countURL="http://vip.stock.finance.sina.com.cn/quotes_service/api/jsonp.php/getTotalPage/Market_Center.getNameCount?node="+forex_whhq.hqNode;

            /*var theScript=$("<script>").attr("src",countURL);
            $("head").append(theScript);
            theScript.remove();*/

            forex_whhq.totalPageScript = document.createElement("script");
            forex_whhq.totalPageScript.setAttribute("src",countURL);
            document.body.insertBefore(forex_whhq.totalPageScript, document.body.childNodes[0]);
        },
        ...
        getSymbol:function(){
            var symbolURL="http://vip.stock.finance.sina.com.cn/quotes_service/api/jsonp.php/symbolData/Market_Center.getNameList?_s_r_a=init"+"&node="+forex_whhq.hqNode;

            /*var theScript=$("<script>").attr("src",symbolURL);
            $("head").append(theScript);
            theScript.remove();*/

            forex_whhq.codeDataScript=document.createElement("script");
            forex_whhq.codeDataScript.setAttribute("src",symbolURL);
            document.body.insertBefore(forex_whhq.codeDataScript, document.body.childNodes[0]);
        },

        /*symbolData function(data){
            //删除掉刚才请求接口时创建的script的标签
            document.body.removeChild(forex_whhq.codeDataScript);

            forex_whhq.symbolArr=[];
            for(var i=0;i<data.length;i++){
                forex_whhq.symbolArr.push(data[i].symbol);
            }
            forex_whhq.symbolArr.sort();

            forex_whhq.getTableData();
        },*/

        getTableData:function(){
            var dataURL="http://hq.sinajs.cn/?_="+Math.random()+"&list="+forex_whhq.symbolArr.toString();
            ...
            forex_whhq.tableDataScript=script;
        },

        pushIntoArray:function(){
            //将数据放在二维数组里
            for(var i=0;i<forex_whhq.symbolArr.length;i++){
                var currentArray=new Array();
                currentArray.push(forex_whhq.symbolArr[i]);//代码

                var currentDataArr=eval("hq_str_"+forex_whhq.symbolArr[i]).split(",");

                currentArray.push(currentDataArr[9]);//名称
                currentArray.push(currentDataArr[1]);//最新价

                var zdf=parseFloat(((parseFloat(currentDataArr[1])-parseFloat(currentDataArr[3]))/parseFloat(currentDataArr[3]))*100).toFixed(2);
                var zde=parseFloat(parseFloat(currentDataArr[1])-parseFloat(currentDataArr[3])).toFixed(4);

                currentArray.push(zdf);//涨跌幅
                currentArray.push(zde);//涨跌额

                currentArray.push(currentDataArr[5]);//开盘价
                currentArray.push(currentDataArr[6]);//最高价
                currentArray.push(currentDataArr[7]);//最低价
                currentArray.push(currentDataArr[4]);//振幅
                currentArray.push(currentDataArr[3]);//昨收价
                currentArray.push(currentDataArr[8]);//买入价
                currentArray.push(currentDataArr[2]);//卖出价

                //console.log(currentArray);

                forex_whhq.totalArray.push(currentArray);
            }
        },
        ...
        insertTableData:function(){
            ...
            if(page*number>forex_whhq.symbolArr.length){
                totalLength=forex_whhq.symbolArr.length;
            }
            else{
            ...
            for(var i=beginIndex;i<totalLength;i++){
                var trDataArr=forex_whhq.totalArray[i];
            ...
            $(forex_whhq.tableDataScript).remove();
        },
...
function getTotalPage(data){
    try{
        //删除掉刚才请求接口时创建的script的标签
        document.body.removeChild(forex_whhq.totalPageScript);
    }
    catch(error){
        //do nothing
    }

    forex_whhq.dataCount=parseInt(data);
