...
$('#ggbdID').hide();
	var config={
		...
		url: {
			...
			changeMentioned:{
				descURL: "http://data.finance.sina.com.cn/api/openapi.php/WeiboReferService.getRefChange?order=DESC&type=CN&num=10&callback=$cb",
				ascURL:"http://data.finance.sina.com.cn/api/openapi.php/WeiboReferService.getRefChange?order=ASC&type=CN&num=10&callback=$cb"
			},
            ggbd:{
...
function insight_wbyq(stockCode){
    ...
    if(ex.test(stockCode)){
		...
		function gprdChart(){
				var _myChart=echarts.init(document.getElementById(config.dom.gprd));
				var _option={
						tooltip:{
								    trigger: 'axis',
				...
				var _symbol='sz300033',_day='',_dk,_sdk;
		            ...
		            wn_varLoader(url.hqURL.replace("$symbol",_symbol).replace("$random",Math.random()),function(){
		                ...
		                wn_varLoader(url.kURL.replace('$symbol',_symbol).replace('$day',_day),function(){
		                    ...
		                    for(var ii=0;ii<rlen;ii++){
		                        ...
		                        if(xtime1==null||xtime1=='') alert(xtime1+"  ddd  "+ _dk[i]);
                    ...
                    if (selected['微博深市股票热度值']) {
                        ...
                        if (!added) {
                            ...
                            setTimeout(function (){
                                ...
                                // console.log(config.datas.stockHeat.k_sz)
                               // console.log(config.datas.stockHeat.sz)
                                _option.series=[
                                { name:'微博深市股票热度值',
                                    type:'line',
                                    yAxisIndex:1,
			...
			var slchart=new gprdChart();
				slchart.init();
		//console.log(config.datas.stockHeat)
		///
...
// 微博股票提及数热力图
	...
	for(var i=0;i<legendNum;i++){
		...
		lgdom.id='qhld'+i
		lgdom.style.cssFloat='left';
		lgdom.style.styleFloat='left';
	...
	function count(d_){
     ...
     //维维股份
    //微博个股提及
     //if(window._cate.length!=0){$('#wbyqID').hide();
     //                       $('#ggbdID').show()
    //     }
    function ggbdChart(){
        var _myChart=echarts.init(document.getElementById(config.dom.ggbd));
            var _option={
                            tooltip:{
                                trigger: 'axis',
                                axisPointer:{
            ...
            function _init(){
                var curl=config.url.ggbd.ggURL.replace('$ggcb',window._cate[0]).replace('$cb',"var "+config.params.cb+"=");              
               
                wn_varLoader(curl,function(){                
                    data_ggbd=window[config.params.cb];    
                    _loadedData(data_ggbd);
                    _loadedData_symbol();
                });
            ...
            //个股K线
            var _ssymbol=window._cate,_sday='',_sdk;//window._cate 全局变量，指insight-wbyq/后的股票代码
                    ...
                    wn_varLoader(url.kURL.replace('$symbol',_ssymbol).replace('$day',_ssday),function(){
                      ...
                      //  console.log(_sdk);
                        ...
                        wn_varLoader(config.url.ggbd.gareaURL.replace('$gCode',_ssymbol).replace('$cb','var '+config.params.area+'='),function(){
                                ...
                                // $("#ggbdCode").html("<h3><a href='"+ggLINK+"'"+ggbdcode+"</a></h3>");
                     ...
                     //  console.log("截止日期 2014.5.27");
                        for(var i in _sdk){
                           ...
                           // ttr.push(xclose);
                            ...
                            //config.times.stockHeat.k_time.push(_date);

                         }};
            //删除微博假期数据
                       // console.log(config.datas.ggbd.k);
           ...
           var ecConfig =echarts.config.EVENT;
            function clickes(param){
                    ...
                    if (selected['全部情感']) {
                        ...
                        added = false;
                        while (len--) {
                            if (_option.series[len].name == '全部情感') {
                                // 已经添加
                                added = true;
                                break;
                            }
                    ...
                    if (selected['正面情感']) {
                        ...
                        added = false;
                        while (len--) {
                            if (_option.series[len].name == '正面情感') {
                                // 已经添加
                                added = true;
                                break;
                            }
                    ...
                    if (selected['负面情感']) {
                        ...
                        added = false;
                        while (len--) {
                            if (_option.series[len].name == '负面情感') {
                                // 已经添加
                                added = true;
                                break;
                            }
                    ...
                    if (selected['中性情感']) {
                        ...
                        added = false;
                        while (len--) {
                            if (_option.series[len].name == '中性情感') {
                                // 已经添加
                                added = true;
                                break;
                            }
	...
	function bdssChart(){
		...
		var _myChart=echarts.init(document.getElementById(config.dom.desc));
		function _init(){
		...
		function _loadedData(){
			...
			for(var i=0;i<data_bdss.result.data.length;i++){
				 ...
				 var change=data_bdss.result.data[i].CHANEG;					 				 
		...
		var _myChart=echarts.init(document.getElementById(config.dom.asc));
			...
			function _loadedData(){
				...
				for(var i=0;i<data_bdxj.result.data.length;i++){
					 ...
					 var change=data_bdxj.result.data[i].CHANEG;
